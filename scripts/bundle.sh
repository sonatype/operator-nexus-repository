...
cp -rv deploy/olm-catalog/nexus-repository-ha-operator-certified/* bundle
...
latest_version=$(cat nexus-repository-ha-operator-certified.package.yaml \
                     | grep currentCSV: \
                     | sed 's/.*nexus-repository-ha-operator-certified.v//')
...
docker build . \
       ...
       -t nexus-repository-ha-operator-bundle:$latest_version

docker tag \
       nexus-repository-ha-operator-bundle:$latest_version \
       scan.connect.redhat.com/${projectId}/nexus-repository-ha-operator-bundle:${latest_version}-${bundleNumber}
